  ...
  return <main className="min-h-screen bg-[#f5f1e7] text-[#1f3b2c]">
    ...
    <section id="chai-rieng" className="scroll-mt-8 px-5 pb-20 pt-16 sm:px-8 lg:px-12 lg:pb-28 lg:pt-24"><div className="mx-auto max-w-7xl"><div className="max-w-2xl"><p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#ad8e50]">{copy.menu.eyebrow}</p><h1 className="font-primary type-h1">{copy.menu.shopTitle}</h1><p className="mt-5 max-w-xl text-[15px] leading-7 text-[#697363]">{copy.menu.shopSupporting}</p><p className="mt-2 max-w-xl text-[14px] leading-6 text-[#697363]">{copy.menu.shopTagline}</p></div>
      <nav className="mt-12 flex flex-wrap items-center gap-2 border-y border-[#1f3b2c]/14 py-3" aria-label={`${copy.menu.selectionCollection} / ${copy.menu.packagesCollection}`}>
        <button type="button" aria-current={collectionTab === "drinks" ? "page" : undefined} onClick={() => scrollToSection("chai-rieng", "drinks")} className={`rounded-full px-4 py-2 text-[11px] font-medium uppercase tracking-[0.08em] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] ${collectionTab === "drinks" ? "bg-[#1f3b2c] text-[#f5f1e7]" : "text-[#697363] hover:text-[#1f3b2c]"}`}>{copy.menu.selectionCollection}<span className="ml-2 opacity-70">{MENU_PRODUCTS.length}</span></button>
        <button type="button" aria-current={collectionTab === "packages" ? "page" : undefined} onClick={() => scrollToSection("morning-packages", "packages")} className={`rounded-full px-4 py-2 text-[11px] font-medium uppercase tracking-[0.08em] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] ${collectionTab === "packages" ? "bg-[#1f3b2c] text-[#f5f1e7]" : "text-[#ad8e50] hover:bg-[#eee6d4]"}`}>{copy.menu.packagesCollection}<span className="ml-2 opacity-70">{MORNING_PACKAGES.length}</span></button>
      </nav>
      <div className="mt-8 flex flex-col gap-5 border-b border-[#1f3b2c]/14 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#ad8e50]">{copy.menu.selectionCollection}</p>
      ...
      <div className="grid gap-x-8 gap-y-14 pt-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filteredProducts.map((product) => <Link key={product.code} href={`/shop/${product.code.toLowerCase()}`} className="shop-card group relative block border-b border-[#1f3b2c]/14 pb-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] focus-visible:ring-offset-4 focus-visible:ring-offset-[#f5f1e7]"><div className="relative h-[350px] overflow-hidden rounded-[12px] bg-[#e8ead9] sm:h-[390px]"><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#173527]/12 via-transparent to-transparent" /><div className="shop-bottle-image absolute inset-x-0 bottom-0 z-20 flex h-[340px] items-end justify-center sm:h-[380px]" style={{ transform: `scale(${product.scale})`, transformOrigin: "50% 100%" }}><img src={product.image} alt={`${localizedProductName(product, locale)} — Morning Green`} className="h-[325px] w-[230px] object-contain object-bottom drop-shadow-[0_22px_16px_rgba(5,36,25,0.16)] sm:h-[365px] sm:w-[250px]" /></div><span className="shop-hover-cta pointer-events-none absolute bottom-4 left-4 z-40 inline-flex translate-y-2 items-center gap-1.5 text-[11px] font-medium uppercase tracking-[0.08em] text-[#1f3b2c]">{copy.action.viewDetails} <span className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-[#1f3b2c]/35"><Plus size={11} strokeWidth={1.5} /></span></span></div><div className="pt-4"><h2 className="type-h3 text-[#1f3b2c]">{localizedProductName(product, locale)}</h2><p className="mt-1 text-[12px] font-medium text-[#526151]">{locale === "vi" ? ENGLISH_PRODUCT_NAMES[product.code] : product.name}</p><p className="mt-2 text-[13px] font-medium text-[#1f3b2c]">{copy.menu.price}: {priceForProduct(product)}</p><p className="mt-1 text-[11px] font-medium leading-5 tracking-[0.015em] text-[#ad8e50]">{localizedDescriptor(product.ingredients, locale)}</p></div></Link>)}</div>{filteredProducts.length === 0 && <div className="mt-8 rounded-2xl border border-dashed border-[#1f3b2c]/20 px-6 py-12 text-center text-sm text-[#697363]">{copy.menu.noResults}</div>}</div></section>
    <section id="morning-packages" className="scroll-mt-8 border-t border-[#1f3b2c]/14 bg-[#e8e4d7] px-5 py-20 sm:px-8 lg:px-12 lg:py-28" aria-labelledby="morning-packages-title" data-collection="MORNING GREEN PACKAGES"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-6 border-b border-[#1f3b2c]/14 pb-8 lg:flex-row lg:items-end"><div><p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#ad8e50]">{copy.menu.packagesCollection}</p><h2 id="morning-packages-title" className="mt-4 max-w-2xl font-primary type-h2 text-[#1f3b2c]">{copy.menu.packagesTitle}</h2></div><div className="max-w-sm"><p className="text-[14px] leading-7 text-[#697363]">{copy.menu.packagesBody}</p><button type="button" onClick={() => scrollToSection("chai-rieng", "drinks")} className="mt-5 inline-flex items-center rounded-full border border-[#1f3b2c]/20 px-4 py-2.5 text-[11px] font-medium uppercase tracking-[0.08em] text-[#1f3b2c] transition hover:border-[#ad8e50] hover:text-[#ad8e50] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50]">{copy.menu.packageBackSelection}</button></div></div><div className="mt-8 border-y border-[#1f3b2c]/10 py-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-2" role="radiogroup" aria-label={copy.menu.packageFilterAria}>{packageDayOptions.map((option) => <button key={option.value} type="button" role="radio" aria-checked={packageDay === option.value} onClick={() => { setPackageDay(option.value); setCollectionTab("packages"); }} className={`rounded-full border px-3.5 py-2 text-[11px] font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] ${packageDay === option.value ? "border-[#1f3b2c] bg-[#1f3b2c] text-[#f5f1e7]" : "border-[#1f3b2c]/15 text-[#697363] hover:border-[#ad8e50] hover:text-[#1f3b2c]"}`}>{option.label}</button>)}</div></div></div><div className="mt-12 grid gap-x-14 lg:grid-cols-2">{visiblePackages.map((packageItem) => { const visual = packageVisuals[packageItem.code]; return <article key={packageItem.code} className="overflow-hidden rounded-[18px] border border-[#1f3b2c]/12 bg-[#f5f1e7] shadow-[0_10px_24px_rgba(23,53,39,0.05)]"><div className="aspect-[3/2] overflow-hidden bg-white"><img src={visual.src} alt={`${localizedPackageName(packageItem, locale)} — ${localizedPackageFormat(packageItem, locale)}`} className="h-full w-full object-contain object-center" loading="lazy" /></div><div className="min-h-[156px] p-6"><div className="flex items-start justify-between gap-4"><span className="font-ui text-2xl font-medium text-[#ad8e50]/80">{packageItem.code.slice(2)}</span><span className="shrink-0 whitespace-nowrap pt-1 font-ui text-[18px] font-medium text-[#1f3b2c]">{packageItem.price}</span></div><h3 className="mt-5 type-h3 text-[#1f3b2c]">{localizedPackageName(packageItem, locale)}</h3><div className="mt-4 flex flex-wrap items-baseline gap-x-2 gap-y-1 border-t border-[#1f3b2c]/10 pt-4"><span className="shrink-0 text-[9px] font-semibold uppercase tracking-[0.16em] text-[#ad8e50]">{copy.menu.packageFormat}</span><p className="text-[13px] font-medium leading-6 text-[#697363]">{localizedPackageFormat(packageItem, locale)}</p></div><a href={FACEBOOK_LINK} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.08em] text-[#1f3b2c] transition hover:text-[#ad8e50] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] focus-visible:ring-offset-2"><Send size={14} strokeWidth={1.6} />{copy.menu.packageAdvice}</a></div></article>; })}</div></div></section>
