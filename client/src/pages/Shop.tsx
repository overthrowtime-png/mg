...
type ShopBottleCalibration = { scale: number; bottomPad: number };

// Source PNGs share the same 1086x1448 canvas, but their visible bottle bounds do not.
// These values normalize the visible bottle height and compensate for transparent bottom padding.
const SHOP_BOTTLE_CALIBRATION: Record<string, ShopBottleCalibration> = {
  S01: { scale: 1.078, bottomPad: 58 },
  S02: { scale: 1.080, bottomPad: 53 },
  S03: { scale: 1.042, bottomPad: 52 },
  S04: { scale: 1.038, bottomPad: 79 },
  S05: { scale: 1.138, bottomPad: 64 },
  S06: { scale: 1.083, bottomPad: 80 },
  S07: { scale: 1.000, bottomPad: 19 },
  S08: { scale: 1.110, bottomPad: 46 },
  S09: { scale: 1.106, bottomPad: 46 },
  S10: { scale: 1.121, bottomPad: 77 },
  S11: { scale: 1.060, bottomPad: 28 },
  S12: { scale: 1.079, bottomPad: 40 },
  J01: { scale: 1.134, bottomPad: 65 },
  J02: { scale: 1.145, bottomPad: 88 },
  J03: { scale: 1.099, bottomPad: 47 },
  J04: { scale: 1.083, bottomPad: 53 },
  J05: { scale: 1.067, bottomPad: 60 },
  J06: { scale: 1.058, bottomPad: 36 },
  J07: { scale: 1.099, bottomPad: 78 },
};

function getShopBottleStyle(code: string): React.CSSProperties {
  const calibration = SHOP_BOTTLE_CALIBRATION[code] ?? { scale: 1, bottomPad: 0 };
  return {
    bottom: `-${Math.round(calibration.bottomPad * calibration.scale * 365 / 1448)}px`,
    transform: `scale(${calibration.scale})`,
