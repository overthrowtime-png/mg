...
import { FACEBOOK_LINK } from "@shared/morningGreen";
import { ENGLISH_PRODUCT_NAMES, MENU_PRODUCTS, MORNING_PACKAGES, localizedDescriptor, priceForProduct } from "../lib/morningGreenCatalog";
...
export default function Shop() {
  ...
  const sortPrefix = locale === "vi" ? "Sắp xếp" : locale === "en" ? "Sort" : "排序";
  const scrollToSection = (id: "chai-rieng" | "morning-packages", tab: CollectionTab) => {
    setCollectionTab(tab);
    document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  const packageDayOptions: Array<{ value: PackageDay; label: string }> = locale === "vi" ? [{ value: "all", label: "Tất cả" }, { value: "1", label: "1 ngày" }, { value: "3", label: "3 ngày" }, { value: "5", label: "5 ngày" }, { value: "7", label: "7 ngày" }, { value: "14", label: "14 ngày" }] : locale === "en" ? [{ value: "all", label: "All" }, { value: "1", label: "1 day" }, { value: "3", label: "3 days" }, { value: "5", label: "5 days" }, { value: "7", label: "7 days" }, { value: "14", label: "14 days" }] : [{ value: "all", label: "全部" }, { value: "1", label: "1 天" }, { value: "3", label: "3 天" }, { value: "5", label: "5 天" }, { value: "7", label: "7 天" }, { value: "14", label: "14 天" }];
  ...
  const packageVisuals: Record<string, { src: string; alt: string }> = {
    MP01: { src: "/morning-package-5-bottles.png", alt: "Năm chai Morning Green trong Gói 1 Ngày · 5 Chai" },
    MP02: { src: "/morning-package-6-bottles.png", alt: "Sáu chai Morning Green trong Gói 1 Ngày · 6 Chai" },
    MP03: { src: "/morning-package-5-bottles.png", alt: "Năm chai Morning Green cho Gói 3 Ngày" },
    MP04: { src: "/morning-package-5-bottles.png", alt: "Năm chai Morning Green mỗi ngày trong Gói 5 Ngày" },
    MP05: { src: "/morning-package-5-bottles.png", alt: "Năm chai Morning Green mỗi ngày trong Gói 7 Ngày" },
    MP06: { src: "/morning-package-5-bottles.png", alt: "Năm chai Morning Green mỗi ngày trong Gói 14 Ngày" },
    MP07: { src: "/morning-package-2-bottles.png", alt: "Hai chai Morning Green mỗi ngày trong Gói 5 Ngày" },
    MP08: { src: "/morning-package-2-bottles.png", alt: "Hai chai Morning Green mỗi ngày trong Gói 7 Ngày" },
    MP09: { src: "/morning-package-3-bottles.png", alt: "Ba chai Morning Green mỗi ngày trong Gói 5 Ngày" },
    MP10: { src: "/morning-package-3-bottles.png", alt: "Ba chai Morning Green mỗi ngày trong Gói 7 Ngày" },
  };

  return <main className="min-h-screen bg-[#f5f1e7] text-[#1f3b2c]">
    <header className="border-b border-[#1f3b2c]/10 bg-[#f5f1e7] px-5 py-4 sm:px-8 lg:px-12"><div className="mx-auto flex max-w-7xl items-center justify-between gap-5"><Link href="/" className="flex items-center gap-3"><img src="/morning-green-logo-160.png" alt="Morning Green" className="h-12 w-12 rounded-[3px] object-contain" /><span className="hidden text-[11px] font-semibold uppercase tracking-[0.2em] text-[#697363] sm:block">Morning Green / Shop</span></Link><div className="flex items-center gap-5"><LocaleSwitcher locale={locale} onChange={setLocale} /><Link href="/" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#1f3b2c] transition hover:text-[#ad8e50]">{locale === "vi" ? "Về trang chủ" : locale === "en" ? "Back home" : "返回首页"}</Link></div></div></header>
    <section id="chai-rieng" className="scroll-mt-8 px-5 pb-20 pt-16 sm:px-8 lg:px-12 lg:pb-28 lg:pt-24"><div className="mx-auto max-w-7xl"><div className="max-w-2xl"><p className="mb-4 text-[10px] font-semibold uppercase tracking-[0.28em] text-[#ad8e50]">{copy.menu.eyebrow}</p><h1 className="font-serif text-[clamp(2.8rem,6vw,5.4rem)] font-normal leading-[0.96] tracking-[-0.04em]">{copy.menu.shopTitle}</h1><p className="mt-5 max-w-xl text-[15px] leading-7 text-[#697363]">{copy.menu.shopSupporting}</p><p className="mt-2 max-w-xl text-[13px] italic leading-6 text-[#697363]">{copy.menu.shopTagline}</p></div>
      ...
      <div className="grid gap-x-8 gap-y-14 pt-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filteredProducts.map((product) => <Link key={product.code} href={`/shop/${product.code.toLowerCase()}`} className="shop-card group relative block border-b border-[#1f3b2c]/14 pb-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] focus-visible:ring-offset-4 focus-visible:ring-offset-[#f5f1e7]"><div className="relative h-[350px] overflow-hidden rounded-[12px] bg-[#e8ead9] sm:h-[390px]"><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#173527]/12 via-transparent to-transparent" /><div className="shop-bottle-image absolute inset-x-0 bottom-0 z-20 flex h-[340px] items-end justify-center sm:h-[380px]" style={{ transform: `scale(${product.scale})`, transformOrigin: "50% 100%" }}><img src={product.image} alt={`Chai ${product.name} Morning Green`} className="h-[325px] w-[230px] object-contain object-bottom drop-shadow-[0_22px_16px_rgba(5,36,25,0.16)] sm:h-[365px] sm:w-[250px]" /></div><span className="shop-hover-cta pointer-events-none absolute bottom-4 left-4 z-40 inline-flex translate-y-2 items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#1f3b2c]">{copy.action.viewDetails} <span className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-[#1f3b2c]/35"><Plus size={11} strokeWidth={1.5} /></span></span></div><div className="pt-4"><h2 className="font-serif text-[19px] font-medium leading-[1.15] tracking-[-0.018em] text-[#1f3b2c]">{product.name}</h2><p className="mt-1 text-[12px] font-medium text-[#526151]">{ENGLISH_PRODUCT_NAMES[product.code]}</p><p className="mt-2 text-[13px] font-semibold text-[#1f3b2c]">{copy.menu.price}: {priceForProduct(product)}</p><p className="mt-1 text-[11px] font-medium leading-5 tracking-[0.015em] text-[#ad8e50]">{localizedDescriptor(product.ingredients, locale)}</p></div></Link>)}</div>{filteredProducts.length === 0 && <div className="mt-8 rounded-2xl border border-dashed border-[#1f3b2c]/20 px-6 py-12 text-center text-sm text-[#697363]">{copy.menu.noResults}</div>}</div></section>
    <section id="morning-packages" className="scroll-mt-8 border-t border-[#1f3b2c]/14 bg-[#e8e4d7] px-5 py-20 sm:px-8 lg:px-12 lg:py-28" aria-labelledby="morning-packages-title" data-collection="MORNING GREEN PACKAGES"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-6 border-b border-[#1f3b2c]/14 pb-8 lg:flex-row lg:items-end"><div><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#ad8e50]">MORNING GREEN PACKAGES</p><h2 id="morning-packages-title" className="mt-4 max-w-2xl font-serif text-[clamp(2.2rem,4.5vw,4.1rem)] font-normal leading-[1] tracking-[-0.035em] text-[#1f3b2c]">Chọn gói phù hợp với bạn.</h2></div><div className="max-w-sm"><p className="text-[14px] leading-7 text-[#697363]">Từ một ngày làm quen đến hành trình duy trì đều đặn, mỗi gói được thiết kế để Morning Green dễ dàng đi cùng nhịp sống của bạn.</p><button type="button" onClick={() => scrollToSection("chai-rieng", "drinks")} className="mt-5 inline-flex items-center rounded-full border border-[#1f3b2c]/20 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#1f3b2c] transition hover:border-[#ad8e50] hover:text-[#ad8e50] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50]">MORNING GREEN SELECTION</button></div></div><div className="mt-8 border-y border-[#1f3b2c]/10 py-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-2" role="radiogroup" aria-label={locale === "vi" ? "Lọc MORNING GREEN PACKAGES theo số ngày" : locale === "en" ? "Filter MORNING GREEN PACKAGES by duration" : "按天数筛选 MORNING GREEN PACKAGES"}>{packageDayOptions.map((option) => <button key={option.value} type="button" role="radio" aria-checked={packageDay === option.value} onClick={() => { setPackageDay(option.value); setCollectionTab("packages"); }} className={`rounded-full border px-3.5 py-2 text-[11px] font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] ${packageDay === option.value ? "border-[#1f3b2c] bg-[#1f3b2c] text-[#f5f1e7]" : "border-[#1f3b2c]/15 text-[#697363] hover:border-[#ad8e50] hover:text-[#1f3b2c]"}`}>{option.label}</button>)}</div></div></div><div className="mt-12 grid gap-x-14 lg:grid-cols-2">{visiblePackages.map((packageItem) => { const visual = packageVisuals[packageItem.code]; return <article key={packageItem.code} className="overflow-hidden rounded-[18px] border border-[#1f3b2c]/12 bg-[#f5f1e7] shadow-[0_10px_24px_rgba(23,53,39,0.05)]"><div className="aspect-[3/2] overflow-hidden bg-white"><img src={visual.src} alt={visual.alt} className="h-full w-full object-contain object-center" loading="lazy" /></div><div className="min-h-[156px] p-6"><div className="flex items-start justify-between gap-4"><span className="font-serif text-2xl text-[#ad8e50]/80">{packageItem.code.slice(2)}</span><span className="shrink-0 whitespace-nowrap pt-1 font-serif text-[18px] text-[#1f3b2c]">{packageItem.price}</span></div><h3 className="mt-5 font-serif text-[clamp(1.2rem,2.2vw,1.65rem)] leading-[1.12] tracking-[-0.015em] text-[#1f3b2c]">{packageItem.name}</h3><div className="mt-4 flex flex-wrap items-baseline gap-x-2 gap-y-1 border-t border-[#1f3b2c]/10 pt-4"><span className="shrink-0 text-[9px] font-semibold uppercase tracking-[0.16em] text-[#ad8e50]">Quy cách</span><p className="text-[13px] font-medium leading-6 text-[#697363]">{packageItem.format}</p></div><a href={FACEBOOK_LINK} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#1f3b2c] transition hover:text-[#ad8e50] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] focus-visible:ring-offset-2"><Send size={14} strokeWidth={1.6} />{locale === "vi" ? "Nhận tư vấn qua Facebook" : locale === "en" ? "Get advice on Facebook" : "通过 Facebook 获取咨询"}</a></div></article>; })}</div></div></section>
  </main>;
}
