...
import { optimizedAsset } from "@shared/morningGreen";
...
export default function Story() {
  ...
  return <main className="story-page min-h-screen overflow-x-hidden bg-[#f5f1e7] text-[#1f3b2c]">
    <header className={`story-site-header fixed inset-x-0 top-0 z-40 border-b border-white/[0.07] text-[#f5f1e7] transition-[height,background-color,backdrop-filter] duration-[280ms] ease-out ${isHeaderCompact ? "h-[68px] bg-[#173527]/92 backdrop-blur-md lg:h-[72px]" : "h-[72px] bg-[#173527] lg:h-[90px]"}`}>
      <div className="mx-auto grid h-full w-full max-w-[1320px] grid-cols-[auto_1fr_auto] items-center gap-5 px-5 sm:px-8 lg:px-10">
        <a href="/" className="group inline-flex items-center gap-3 focus:outline-none focus-visible:ring-2 focus:ring-[#c9af77] focus:ring-offset-2 focus:ring-offset-[#173527]">
          <img src={optimizedAsset("/morning-green-logo-160.png")} alt="Morning Green" decoding="async" className={`w-auto rounded-[7px] border border-white/[0.12] object-contain transition-[height] duration-[280ms] ease-out ${isHeaderCompact ? "h-[48px] lg:h-[56px]" : "h-[52px] sm:h-[54px] lg:h-[72px]"}`} />
        </a>
    ...
    <section className="story-hero relative flex min-h-[85svh] items-end overflow-hidden bg-[#173527] text-[#f5f1e7] lg:min-h-[82vh]">
      ...
      <div className="story-hero-art absolute inset-0 bg-cover opacity-80 lg:opacity-100" style={{ backgroundImage: `url('${optimizedAsset("/story-hero-user-composite.png")}')` }} aria-hidden="true" />
    ...
    <section id="story-chapter-02" className="bg-[#f5f1e7] px-5 py-24 sm:px-8 lg:px-12 lg:py-36"><div className="mx-auto grid max-w-7xl gap-16 xl:grid-cols-[0.75fr_1.25fr] xl:items-start"><div data-story-reveal className="story-reveal min-w-0"><p className="story-label text-[#ad8e50]">{story.whyEyebrow}</p><h2 className="mt-5 story-h2">{story.whyTitle}</h2></div><div data-story-reveal className="story-reveal min-w-0 max-w-2xl xl:pt-12"><p className="story-lead text-[#1f3b2c]">{story.heroBody}</p><p className="mt-8 story-body text-[#697363]">{story.whyBody}</p></div></div></section>

    <section className="story-dark-chapter relative isolate overflow-hidden bg-[#1f3b2c] px-5 py-24 text-[#f5f1e7] sm:px-8 lg:px-12 lg:py-36"><div className="absolute inset-0 bg-cover bg-center opacity-80 lg:opacity-100" style={{ backgroundImage: `url('${optimizedAsset("/bg-s01-gon-bung-nhe-tenh.png")}')`, backgroundPosition: "center" }} aria-hidden="true" /><div className="absolute inset-0 bg-gradient-to-r from-[#1f3b2c]/96 via-[#1f3b2c]/78 to-[#1f3b2c]/34" aria-hidden="true" /><div className="absolute inset-0 bg-gradient-to-t from-[#1f3b2c]/58 via-transparent to-[#1f3b2c]/18" aria-hidden="true" /><div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_0.8fr] lg:items-end"><div data-story-reveal className="story-reveal"><p className="story-label text-[#c9af77]">{story.greenEyebrow}</p><h2 className="mt-6 max-w-3xl story-h2">Morning<br /><em className="text-[#c9af77]">Green.</em></h2></div><p data-story-reveal className="story-reveal story-body text-[#cbd4c4]">{story.greenBody}</p></div></section>

    <section className="relative overflow-hidden bg-[#e8e4d7]"><div className="story-image-break min-h-[58vh] bg-cover bg-center" style={{ backgroundImage: `linear-gradient(rgba(245,241,231,.04), rgba(23,53,39,.2)), url('${optimizedAsset("/bottle-stage-ingredients.png")}')` }} role="img" aria-label={story.freshnessBody} /><div className="pointer-events-none absolute inset-x-0 bottom-0 h-56 bg-gradient-to-t from-[#173527]/80 via-[#173527]/20 to-transparent" aria-hidden="true" /></section>

    <section className="bg-[#f5f1e7] px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><div className="mx-auto max-w-7xl"><div data-story-reveal className="story-reveal border-b border-[#1f3b2c]/14 pb-10"><p className="story-label text-[#ad8e50]">{story.processEyebrow}</p><h2 className="mt-5 max-w-3xl story-h2">{story.processTitle}</h2></div><div className="mt-14 grid items-start gap-10 sm:grid-cols-2 lg:grid-cols-3">{story.process.map((item) => <div key={item.number} data-story-reveal className="story-reveal flex h-full flex-col border-t border-[#1f3b2c]/14 pt-5"><span className="block h-6 font-ui text-[21px] font-medium leading-none text-[#ad8e50]/80">{item.number}</span><h3 className="mt-8 story-h3 text-[#1f3b2c]">{item.title}</h3><p className="mt-4 story-body-sm text-[#697363]">{item.body}</p></div>)}</div></div></section>
    ...
    <section className="bg-[#f5f1e7] px-5 py-24 sm:px-8 lg:px-12 lg:py-36"><div className="mx-auto max-w-7xl"><div className="max-w-3xl" data-story-reveal><p className="story-reveal story-label text-[#ad8e50]">{story.realEyebrow}</p><h2 className="story-reveal mt-5 story-h2 whitespace-pre-line">{story.realTitle}</h2><p className="story-reveal mt-8 story-body whitespace-pre-line text-[#697363]">{story.realBody}</p></div><div className="mt-14 grid gap-8 border-t border-[#1f3b2c]/14 pt-8 md:grid-cols-3">{story.moments.map((moment, index) => <article key={moment.time} data-story-reveal className="story-reveal"><div className={`relative aspect-[1.15] overflow-hidden ${index === 0 ? "bg-[#e8ead9]" : index === 1 ? "bg-[#e8e4d7]" : "bg-[#e2e8df]"}`}><img src={optimizedAsset(moment.image)} alt={moment.title} loading="lazy" decoding="async" fetchPriority="low" className="h-full w-full object-contain object-center p-2 transition duration-700 hover:scale-[1.02] sm:p-3" /></div><p className="mt-5 font-ui text-[21px] font-medium leading-none text-[#ad8e50]">{moment.time}</p><h3 className="mt-4 story-h3 text-[#1f3b2c]">{moment.title}</h3><p className="mt-3 max-w-xs story-caption text-[#697363]">{moment.body}</p></article>)}</div></div></section>
