...
export default function Story() {
  ...
  return <main className="story-page min-h-screen overflow-x-hidden bg-[#f5f1e7] text-[#1f3b2c]">
    ...
    <section className="relative overflow-hidden bg-[#e8e4d7]"><div className="pointer-events-none absolute inset-x-0 top-0 z-10 px-5 pt-6 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><p className="story-label text-[#1f3b2c]/70">{story.freshnessEyebrow}</p></div></div><div className="story-image-break min-h-[58vh] bg-cover bg-center" style={{ backgroundImage: "linear-gradient(rgba(245,241,231,.04), rgba(23,53,39,.2)), url('/bottle-stage-ingredients.png')" }} role="img" aria-label={story.freshnessBody} /><div className="pointer-events-none absolute inset-x-0 bottom-0 h-56 bg-gradient-to-t from-[#173527]/80 via-[#173527]/20 to-transparent" aria-hidden="true" /></section>

    <section className="bg-[#f5f1e7] px-5 py-20 sm:px-8 lg:px-12 lg:py-28"><div className="mx-auto max-w-7xl"><div data-story-reveal className="story-reveal border-b border-[#1f3b2c]/14 pb-10"><p className="story-label text-[#ad8e50]">{story.processEyebrow}</p><h2 className="mt-5 max-w-3xl story-h2">{story.processTitle}</h2></div><div className="mt-14 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">{story.process.map((item) => <div key={item.number} data-story-reveal className="story-reveal flex h-full flex-col border-t border-[#1f3b2c]/14 pt-5"><span className="block h-6 font-ui text-[21px] font-medium leading-none text-[#ad8e50]/80">{item.number}</span><h3 className="mt-8 story-h3 text-[#1f3b2c]">{item.title}</h3><p className="mt-4 story-body-sm text-[#697363]">{item.body}</p></div>)}</div></div></section>
