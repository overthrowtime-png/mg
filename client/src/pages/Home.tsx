...
const HERO_SLIDES = [
  { image: HERO_IMAGE, background: optimizedAsset("/bottle-stage-ingredients.png"), alt: "Chai Sắc Vóc Tuyệt Mỹ Morning Green giữa rau xanh, dưa leo và ổi tươi", name: "Sắc Vóc Tuyệt Mỹ", scale: 1 },
  { image: optimizedAsset("/S05_NO_LAU_BEN_BI.png"), background: optimizedAsset("/s05-background.png"), alt: "Chai No Lâu Bền Bỉ Morning Green giữa rau bina, cần tây, táo và chuối tươi", name: "No Lâu Bền Bỉ", scale: 1.067 },
  { image: optimizedAsset("/S01_GONBUNGNHETENH.png"), background: optimizedAsset("/bg-s01-gon-bung-nhe-tenh.png"), alt: "Chai Gọn Bụng Nhẹ Tênh Morning Green giữa rau xanh và nguyên liệu tươi", name: "Gọn Bụng Nhẹ Tênh", scale: 1.01 },
  { image: optimizedAsset("/S02_DA_S_NG_D_NG_G_N.png"), background: optimizedAsset("/bg-s02-da-sang-dang-gon.png"), alt: "Chai Da Sáng Dáng Gọn Morning Green giữa rau xanh và trái cây tươi", name: "Da Sáng Dáng Gọn", scale: 1.012 },
  { image: optimizedAsset("/S03_THAI_DOC_THANH_LOC.png"), background: optimizedAsset("/bg-s03-thai-doc-thanh-loc.png"), alt: "Chai Thải Độc Thanh Lọc Morning Green giữa cần tây, chanh và rau xanh", name: "Thải Độc Thanh Lọc", scale: 0.977 },
] as const;
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f5f1e7] text-[#1f3b2c]">
      <header className={`fixed inset-x-0 top-0 z-40 border-b border-white/[0.07] text-[#f5f1e7] transition-[height,background-color,backdrop-filter] duration-[280ms] ease-out ${isHeaderCompact ? "h-[68px] bg-[#173527]/92 backdrop-blur-md lg:h-[72px]" : "h-[72px] bg-[#173527] lg:h-[90px]"}`}>
        <div className={`mx-auto grid h-full w-full max-w-[1320px] grid-cols-[auto_1fr_auto] items-center gap-5 px-5 transition-[padding] duration-[280ms] ease-out sm:px-8 lg:px-10 ${isHeaderCompact ? "lg:px-10" : "lg:px-10"}`}>
          ...
          >
            <img
              src={optimizedAsset("/morning-green-logo-160.png")}
              alt="Morning Green"
              decoding="async"
          ...
          <div className="order-1 relative min-h-[480px] lg:order-2 lg:min-h-[620px]">
            <div aria-label={`${copy.hero.slideLabel}: ${activeHero.name}`} className="absolute inset-x-6 top-0 h-[430px] overflow-hidden rounded-[45%_45%_18%_18%] bg-[#efe8d7] shadow-[0_22px_60px_rgba(5,36,25,0.22)] lg:inset-x-16 lg:h-[560px]">
              {HERO_SLIDES.map((slide, index) => (
                <div key={`${slide.name}-background`} aria-hidden="true" className={`absolute inset-0 bg-cover bg-center transition-opacity duration-[1400ms] ease-in-out ${index === activeHeroIndex ? "opacity-100" : "opacity-0"}`} style={index === activeHeroIndex ? { backgroundImage: `linear-gradient(rgba(247,242,224,0.08), rgba(247,242,224,0.08)), url('${slide.background}')` } : undefined} />
              ))}
              {HERO_SLIDES.map((slide, index) => (
                <img key={slide.name} src={slide.image} alt={slide.alt} loading={index === activeHeroIndex ? "eager" : "lazy"} decoding="async" fetchPriority={index === activeHeroIndex ? "high" : "low"} style={{ transform: `scale(${slide.scale})`, transformOrigin: "50% 100%" }} className={`absolute inset-0 z-20 h-full w-full object-contain object-bottom drop-shadow-[0_24px_18px_rgba(5,36,25,0.28)] transition-opacity duration-[1400ms] ease-in-out ${index === activeHeroIndex ? "opacity-100" : "opacity-0"}`} />
              ))}
      ...
      <section id="menu" className="relative scroll-mt-28 overflow-hidden bg-[#f5f1e7] px-5 pb-6 pt-6 sm:px-8 sm:pb-8 sm:pt-8 lg:px-12 lg:pb-10 lg:pt-10">
        <div className="mx-auto max-w-7xl">
            ...
            {FEATURED_PRODUCTS.map((product) => (
              <Link key={product.code} href={`/shop/${product.code.toLowerCase()}`} className="shop-card group relative block border-b border-[#1f3b2c]/14 pb-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#ad8e50] focus-visible:ring-offset-4 focus-visible:ring-offset-[#f5f1e7]">
                <div className="relative h-[320px] overflow-hidden rounded-[12px] bg-[#e8ead9] sm:h-[350px]">
                  ...
                  <div className="shop-bottle-image absolute inset-x-0 bottom-0 z-20 flex h-[310px] items-end justify-center sm:h-[340px]" style={{ transform: `scale(${product.scale})`, transformOrigin: "50% 100%" }}><img src={optimizedAsset(product.image)} alt={`Chai ${product.name} Morning Green`} loading="lazy" decoding="async" fetchPriority="low" className="h-[300px] w-[235px] object-contain object-bottom drop-shadow-[0_22px_16px_rgba(5,36,25,0.16)] sm:h-[330px] sm:w-[260px]" /></div>
          ...
          <div className="mt-6 flex justify-center"><a href="/shop" className="inline-flex items-center gap-2 border-b border-[#ad8e50] pb-1 text-[12px] font-semibold uppercase tracking-[0.14em] text-[#1f3b2c] transition hover:text-[#ad8e50]">{copy.menu.viewAll} <ChevronRight size={14} /></a></div>
        </div>
      </section>
