...
function LocaleSwitcher({ locale, onChange }: { locale: Locale; onChange: (next: Locale) => void }) {
  ...
  return (
    <div className="flex items-center gap-2 text-[12px] font-medium tracking-[0.06em] text-[#f5f1e7]" aria-label="Language selector">
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f5f1e7] text-[#1f3b2c]">
      <header className={`fixed inset-x-0 top-0 z-40 border-b border-white/[0.07] text-[#f5f1e7] transition-[height,background-color,backdrop-filter] duration-[280ms] ease-out ${isHeaderCompact ? "h-[68px] bg-[#173527]/92 backdrop-blur-md lg:h-[72px]" : "h-[72px] bg-[#173527] lg:h-[90px]"}`}>
        <div className={`mx-auto grid h-full w-full max-w-[1320px] grid-cols-[auto_1fr_auto] items-center gap-5 px-5 transition-[padding] duration-[280ms] ease-out sm:px-8 lg:px-10 ${isHeaderCompact ? "lg:px-10" : "lg:px-10"}`}>
          <button
            ...
            aria-label="Về đầu trang Morning Green"
          >
            <img
        ...
        {menuOpen && (
          <div id="mobile-site-menu" className="border-t border-white/[0.08] bg-[#173527] px-5 pb-7 pt-4 shadow-[0_18px_30px_rgba(5,36,25,0.18)] lg:hidden">
            ...
            <div className="mt-5 flex items-center justify-between border-t border-white/[0.08] pt-5"><span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#a7c19d]">{copy.localeName}</span><LocaleSwitcher locale={locale} onChange={setLocale} /></div>
          </div>
        )}
      </header>
      ...
      <section id="top" className="relative min-h-[760px] overflow-hidden bg-[#173527] pt-32 text-[#f5f1e7] lg:min-h-[850px]">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-14 px-5 pb-20 pt-16 sm:px-8 lg:grid-cols-[0.88fr_1.12fr] lg:gap-16 lg:px-12 lg:pb-28 lg:pt-24">
          ...
          <div className="order-1 relative min-h-[480px] lg:order-2 lg:min-h-[620px]">
            <div aria-label={`Hero ${activeHero.name}`} className="absolute inset-x-6 top-0 h-[430px] overflow-hidden rounded-[45%_45%_18%_18%] bg-[#efe8d7] shadow-[0_22px_60px_rgba(5,36,25,0.22)] lg:inset-x-16 lg:h-[560px]">
      ...
      <section id="menu" className="relative scroll-mt-28 overflow-hidden bg-[#f5f1e7] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
      ...
      <section id="discovery" className="scroll-mt-28 bg-[#1f3b2c] px-5 py-24 text-[#f5f1e7] sm:px-8 lg:px-12 lg:py-32">
        <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1fr_0.8fr]">
          ...
          <Link href="/shop#morning-packages" aria-label={locale === "vi" ? "Xem Morning Packages" : locale === "en" ? "View Morning Green Packages" : "查看 Morning Green 套餐"} className="group relative mx-auto block w-full max-w-md"><div className="absolute -inset-5 rounded-[35px] border border-[#c9af77]/20 transition group-hover:border-[#c9af77]/45" /><div className="relative rounded-[28px] bg-[#f5f1e7] p-5 text-[#1f3b2c] shadow-2xl transition duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_20px_48px_rgba(5,36,25,0.24)]"><div className="flex items-center justify-between border-b border-[#1f3b2c]/10 pb-4"><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#ad8e50]">Morning Packages</span><span className="text-xs text-[#65735f]">{locale === "vi" ? "1 · 3 · 5 · 7 · 14 ngày" : locale === "en" ? "1 · 3 · 5 · 7 · 14 days" : "1 · 3 · 5 · 7 · 14 天"}</span></div><div className="grid grid-cols-2 gap-2 py-5"><div className="overflow-hidden rounded-[14px] bg-white"><img src="/morning-package-2-bottles.png" alt="Gói Morning Green 2 chai mỗi ngày" className="aspect-[3/2] h-full w-full object-contain" loading="lazy" /></div><div className="overflow-hidden rounded-[14px] bg-white"><img src="/morning-package-3-bottles.png" alt="Gói Morning Green 3 chai mỗi ngày" className="aspect-[3/2] h-full w-full object-contain" loading="lazy" /></div><div className="overflow-hidden rounded-[14px] bg-white"><img src="/morning-package-5-bottles.png" alt="Gói Morning Green 5 chai" className="aspect-[3/2] h-full w-full object-contain" loading="lazy" /></div><div className="overflow-hidden rounded-[14px] bg-white"><img src="/morning-package-6-bottles.png" alt="Gói Morning Green 6 chai" className="aspect-[3/2] h-full w-full object-contain" loading="lazy" /></div></div><div className="flex items-end justify-between border-t border-[#1f3b2c]/10 pt-4"><div><div className="font-serif text-2xl font-medium">Morning Packages</div><div className="mt-1 text-xs text-[#697363]">{copy.discovery.note}</div></div><div className="text-right"><div className="font-serif text-xl font-medium">{copy.discovery.taste}</div></div></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[#1f3b2c]/10 pt-3"><p className="max-w-[18rem] text-[10px] leading-4 text-[#697363]">{locale === "vi" ? "Hình ảnh minh họa. Sản phẩm thực tế có thể thay đổi theo thực đơn." : locale === "en" ? "Illustrative images. Actual products may vary by menu." : "图片仅供参考，实际产品可能因菜单而有所不同。"}</p><span className="inline-flex shrink-0 items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#1f3b2c]">{locale === "vi" ? "Xem Morning Packages" : locale === "en" ? "View packages" : "查看套餐"}<ChevronRight size={14} strokeWidth={1.7} /></span></div></div></Link>
        </div>
      </section>
