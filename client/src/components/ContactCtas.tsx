...
export function ContactCtas({ copy, className = "", layout = "inline", compact = false, facebookIconOnly = false, facebookIconLabel = false, facebookLabelOverride }: ContactCtasProps) {
  ...
  const facebookSizeClass = facebookIconLabel ? (compact ? "min-h-11 text-[11px]" : "min-h-12 text-[12px]") : sizeClass;
  ...
  return (
    <div className={`flex w-full flex-col ${compact ? "gap-2.5" : "gap-3"} ${isStacked ? "items-start" : "sm:flex-row sm:items-center"} ${className}`}>
      ...
      >
        ...
        {facebookIconOnly ? <span className="sr-only">{facebookLabel}</span> : <><span className="whitespace-nowrap">{facebookLabel}</span>{!facebookIconLabel && <span aria-hidden="true" className="text-[14px] transition-transform duration-200 group-hover:translate-x-0.5">↗</span>}</>}
      </a>
